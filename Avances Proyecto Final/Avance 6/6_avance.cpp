...
class jugador{
	public:
		...
		int movi;
		bool bomba;
	jugador(char j,int mov, bool bomb){
		id=j;
		movi=mov;
		bomba = bomb;
	}	
...
void  preguntas(char num, bool ocupado, char rpta){
    if(pregunta[0]==num && ocupado==false){
        ...
        if(respuesta == rpta){ cout<<"La respuesta es correcta, ganaste una bonificación "<<endl; }
        else { cout<<"La respuesta es inccorrecta"<<endl; }
    }
}
void verRegistros(ifstream &lec){
    ...
    while(!lec.eof()){
        ...
        lec>>opciond;
        lec>>opcione;
        if(ocupado_1==false) { preguntas('1', ocupado_1, 'd'); break;}
        else if(ocupado_2==false) { preguntas('2', ocupado_2, 'a');break; }
        else if(ocupado_3==false) { preguntas('3', ocupado_3, 'a');break; }
        else if(ocupado_4==false) { preguntas('4', ocupado_4, 'b');break; }
        else if(ocupado_5==false) { preguntas('5', ocupado_5, 'c');break; }
        else if(ocupado_6==false) { preguntas('6', ocupado_6, 'c');break; }
        else if(ocupado_7==false) { preguntas('7', ocupado_7, 'b');break; }
        else if(ocupado_8==false) { preguntas('8', ocupado_8, 'c');break; }
        else if(ocupado_9==false) { preguntas('9', ocupado_9, 'd');break; }
	    lec>>pregunta; 	
	}
	lec.close();
}
...
void moverJugador(jugador *j, int mov_dado, mapa *map){ // true = bomba activada
	if ( j->bomba==false){
		j->movi += mov_dado;
	    //Verifica limites
		...
		cout << "Jugador "<< j->id << " pierdes este turno, descuida podras jugar nuevamente" << endl;
		j->bomba = false ;
	}
}
void tirarDado(int *mov_dado ,jugador j){
